package com.github.hqxqyly.remex.boot.pagehelper.interceptor;
...
import com.github.hqxqyly.remex.boot.pagehelper.holder.PageCountHolder;

/**
 * 重写PageInterceptor
 * 已调整功能：
 * 1：可设置总数，来让PageHelper直接取此值作为总数，而不是通过sql再查询
 * 
 * 
...
public class RemexPageInterceptor implements Interceptor {
        ...
        try {
            ...
            if (!dialect.skip(ms, parameter, rowBounds)) {
                ...
                if (dialect.beforeCount(ms, parameter, rowBounds)) {
                	//判断是否已有总数
                	Long count = PageCountHolder.get();
                	if (count == null) {
                		//查询总数
